...
import {
  ...
  Text,
  TextInput,
  TouchableNativeFeedback, useWindowDimensions, Button, SafeAreaView
} from "react-native";
...
const AddList = ({ navigation }: any) => {
  ...
  // @ts-ignore
  const [shopList, setShopList]: [ShopList, Function] = useState(params?.list || { name: "", items: [], id: uuidv4() });
  ...
  const removeItem = useCallback(
    (index: number) => {
      setShopList((shopList: ShopList) => ({
        ...shopList,
        items: [...shopList.items.slice(0, index), ...shopList.items.slice(index + 1)]
      }));
    },
    []
  );
  ...
  const setItemName = useCallback(
    (name: string, index: number) => {
      setShopList((shopList: ShopList) => ({
        ...shopList,
        items: [...shopList.items.map((item, i) => index === i ? { ...item, name } : item)]
      }));
    },
    []
  );
  ...
  const setItemQuantity = useCallback(
    (quantity: string, index: number) => {
      setShopList((shopList: ShopList) => ({
        ...shopList,
        items: [...shopList.items.map((item, i) => index === i ? { ...item, quantity } : item)]
      }));
    },
    []
  );
  ...
  const canCreate = useMemo(() => shopList?.items?.length > 0 && !shopList?.items.some(item => (!item?.quantity || !item?.name)), [shopList]);
  // @ts-ignore
  //check if list is passed trough params
  const hasList = useMemo(() => params?.list !== undefined, [params]);
  // @ts-ignore
  //check if list has been updated
  const hasChanged = useMemo(() => !isEqual(params?.list, shopList), [params, shopList] );
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.header}>
        <TextInput placeholder={"Nouvelle Liste"}
                   style={styles.title} onChangeText={setListName}
                   value={shopList?.name} />
        <TouchableNativeFeedback onPress={hasList ? deleteList : addItem}>
          {hasList ?
            <Icon name={"trash-2-outline"} width={24} height={24} fill={"red"} /> :
      ...
      <FlatList
        ...
        renderItem={({ item, index }: { item: ShopItem; index: number }) => (
          <AddItem index={index}
                ...
                handleQuantityChange={(value: string) => setItemQuantity(value, index)}
                {...item} />
        )}
        keyExtractor={(item: ShopItem) => `item-${item.id}`}
      />
      {canCreate && <View style={{
        alignItems: "center",
        ...
        {hasList && hasChanged && <Button title={"Modifier la liste"} onPress={updateList} />}
